import "server-only";

import { cache } from "react";
import { prisma } from "@/lib/prisma";
import {
  DEFAULT_JOIN_CONFIG,
  JoinConfigSchema,
  type JoinConfig,
} from "@/lib/validators/join-config";

/**
 * 站点级配置的读写。
 *
 * 存储：`SiteSetting` 键值表（value 是 Json）。之所以不每种配置建一张表，
 * 是因为配置项会零散地增加；类型安全由这里的 zod 校验保证 —— 数据库
 * 只负责"存住"，不负责"存对"。
 */

/** 「加入我们」配置在 SiteSetting 里的键 */
export const JOIN_CONFIG_KEY = "join";

/**
 * 读「加入我们」配置。
 *
 * 三种情况都退回默认值，绝不让首页因为配置问题挂掉：
 *   1. 从没配置过（没有这一行）
 *   2. 库里存的是旧版本结构 / 被手改坏了（zod 解析失败）
 *   3. 数据库暂时连不上（catch）
 */
export const getJoinConfig = cache(async (): Promise<JoinConfig> => {
  try {
    const row = await prisma.siteSetting.findUnique({
      where: { key: JOIN_CONFIG_KEY },
      select: { value: true },
    });
    if (!row) return DEFAULT_JOIN_CONFIG;

    const parsed = JoinConfigSchema.safeParse(row.value);
    if (!parsed.success) {
      console.warn("[getJoinConfig] 配置结构不合法，已回退默认值");
      return DEFAULT_JOIN_CONFIG;
    }
    return parsed.data;
  } catch (err) {
    console.error("[getJoinConfig]", err);
    return DEFAULT_JOIN_CONFIG;
  }
});

/** 写「加入我们」配置（管理员已校验过权限与内容） */
export async function setJoinConfig(config: JoinConfig): Promise<void> {
  await prisma.siteSetting.upsert({
    where: { key: JOIN_CONFIG_KEY },
    create: { key: JOIN_CONFIG_KEY, value: config },
    update: { value: config },
  });
}

/**
 * 列出可作为「加入我们」目标的已发布问卷。
 * 后台下拉框和 join-state 的校验共用。
 */
export async function listJoinableQuestionnaires() {
  return prisma.questionnaire.findMany({
    where: { status: "PUBLISHED" },
    orderBy: { createdAt: "desc" },
    select: {
      id: true,
      title: true,
      _count: { select: { questions: true, submissions: true } },
    },
  });
}
